...
static void testPrintVars(CMakeInterpreter::InterpreterState& state, std::string_view args) {
...
static void testClearVars(CMakeInterpreter::InterpreterState& state, std::string_view args) {
	for (auto itr = state.m_Variables.rbegin(); itr != state.m_Variables.rend(); ++itr) {
		itr->clear();
	}
}
...
static bool testOneArgument(Tester& tester) {
	for (std::size_t i = 0; i < 2; ++i) {
		if (!s_State.hasNext())
			return false;
...
static bool testTwoArguments(Tester& tester) {
	for (std::size_t i = 0; i < 2; ++i) {
		if (!s_State.hasNext())
			return false;
...
static bool testVar(Tester& tester) {
...
static bool testFunctionScope(Tester& tester) {
	for (std::size_t i = 0; i < 3; ++i) {
		if (!s_State.hasNext())
			return false;
...
static bool testMacroScope(Tester& tester) {
	for (std::size_t i = 0; i < 3; ++i) {
		if (!s_State.hasNext())
			return false;
...
static bool testVarReplacements(Tester& tester) {
